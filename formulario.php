...
<body>
    ...
    <script>
         // Quando um link ou botÃ£o Ã© clicado
         document.querySelectorAll('a, button').forEach(link => {
             link.addEventListener('click', (event) => {
                 event.preventDefault(); // Impede o comportamento padrÃ£o de seguir o link imediatamente
                 const destination = link.getAttribute('href'); // ObtÃ©m o destino do link
                 document.querySelector('body').classList.add('exiting'); // Adiciona a classe .exiting
                 setTimeout(() => {
                     window.location.href = destination; // Redireciona para o destino apÃ³s a transiÃ§Ã£o
                 }, 500); // Tempo da transiÃ§Ã£o em milissegundos (0,5 segundos)
             });
         });
     </script>
</body>
